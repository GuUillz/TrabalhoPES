  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-gradient-to-b from-[#0f172a] to-[#1e3a8a] flex flex-col lg:flex-row overflow-hidden z-0">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 overflow-y-auto">
            ...
            <div className="w-full max-w-3xl flex flex-col gap-6">
                <div className="bg-white/5 backdrop-blur-md rounded-3xl p-6 md:p-8 flex justify-between items-center px-6 md:px-12 shadow-xl border border-white/10 hover:bg-white/10 transition duration-300 group">
                    <div><span className="text-2xl md:text-4xl font-black text-white block mb-1 group-hover:text-pink-300 transition">INTEIRA</span><span className="text-sm md:text-lg text-gray-300 font-bold bg-black/30 px-3 py-1 rounded-lg">R$ 30,00</span></div>
                ...
                <div className="bg-white/5 backdrop-blur-md rounded-3xl p-6 md:p-8 flex justify-between items-center px-6 md:px-12 shadow-xl border border-white/10 hover:bg-white/10 transition duration-300 group">
                    <div><span className="text-2xl md:text-4xl font-black text-white block mb-1 group-hover:text-pink-300 transition">MEIA ENTRADA</span><span className="text-sm md:text-lg text-gray-300 font-bold bg-black/30 px-3 py-1 rounded-lg">R$ 15,00</span></div>
                    <div className="flex items-center gap-6 md:gap-8">
      ...
      <div className="w-full lg:w-96 h-auto lg:h-full bg-black/30 backdrop-blur-xl border-t lg:border-t-0 lg:border-l border-white/10 flex flex-col z-20 shrink-0 shadow-2xl">
        <div className="flex-1 lg:overflow-auto"><OrderSummary movie={state?.movie} session={state?.session} seats={state?.seats} tickets={{ inteira, meia }} total={totalPrice} /></div>
        <div className="p-6 border-t border-white/10 bg-black/20">
             <button onClick={() => navigate('/pagamento', { state: { ...state, tickets: {inteira, meia}, total: totalPrice } })} disabled={currentTotal !== totalSeats} className="w-full bg-pink-400 hover:bg-pink-300 text-blue-950 font-black py-4 rounded-2xl text-lg shadow-xl disabled:opacity-50 disabled:cursor-not-allowed transition hover:scale-105 transform active:scale-95">IR PARA PAGAMENTO</button>
        </div>
      </div>
    </div>
  );
