...
const movies = [
  ...
  { id: 5, title: 'O PROTETOR 3', poster: 'https://placehold.co/300x450/1e3a8a/white?text=Protetor' },
];
...
const Home = () => {
  ...
  const scroll = (direction) => {
    if (scrollContainer.current) {
      ...
      const scrollAmount = window.innerWidth < 768 ? 200 : 400; // Scroll menor em mobile
      current.scrollBy({ left: direction === 'left' ? -scrollAmount : scrollAmount, behavior: 'smooth' });
    }
  };

  return (
    <div className="flex flex-col h-screen w-full items-center justify-center relative overflow-hidden bg-blue-900">
      ...
      <div className="flex flex-row items-center justify-center w-full h-auto z-10 px-2 md:px-12 gap-2 md:gap-4">
        ...
        <div 
            ref={scrollContainer}
            className="flex flex-row gap-4 md:gap-8 items-center overflow-x-auto px-8 py-12 w-full hide-scrollbar scroll-smooth snap-x snap-mandatory"
        >
            {movies.map((movie) => (
            <div 
                key={movie.id} 
                ...
                className="
                    snap-center relative flex-shrink-0 cursor-pointer transition-all duration-300 ease-out rounded-xl overflow-hidden group
                    w-40 h-60             /* Mobile */
                    md:w-56 md:h-80       /* Tablet */
                    lg:w-64 lg:h-96       /* Desktop */
                    hover:scale-105 hover:shadow-[0_0_30px_rgba(255,255,255,0.3)] hover:z-20
                "
            >
