...
const SessionSelection = () => {
  ...
  // Estados para controlar a sessão e o áudio selecionados
  const [selectedSession, setSelectedSession] = useState(null);
  const [selectedAudio, setSelectedAudio] = useState('dublado'); // Padrão: Dublado
            ...
            <div className="text-white flex flex-col items-center lg:items-start text-center lg:text-left w-full max-w-3xl">
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-4 mb-8 text-sm md:text-base font-semibold opacity-90">
                    <span className="bg-black/30 px-4 py-2 rounded-lg border border-white/10">
                        Classificação: <span className="text-yellow-400">{movie?.rating || '18 anos'}</span>
                    </span>
                    <span className="bg-black/30 px-4 py-2 rounded-lg border border-white/10">
                        Duração: <span className="text-yellow-400">{movie?.duration || '120 min'}</span>
                    </span>
                    <span className="bg-black/30 px-4 py-2 rounded-lg border border-white/10">
                        Gênero: <span className="text-yellow-400">{movie?.genre || 'Ação'}</span>
                    </span>
                </div>
                
                {/* FIX: Botões de Áudio com Estado */}
                ...
                {/* FIX: Grid de Horários com cores corrigidas */}
                    ...
                    {sessions.map(sess => {
                        ...
                        return (
                            ...
                            className={`
                                ...
                                ${isSelected 
                                    ? 'bg-pink-400 border-pink-400 scale-110 z-10 shadow-pink-400/40' // Cor Selecionado (Rosa)
                                    : 'bg-black/20 border-white/10 text-gray-300 hover:bg-white/10 hover:border-white/30' // FIX: Cor Não Selecionado (Escura Transparente)
                                }
                            `}
                            >
                            <span className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${isSelected ? 'text-blue-950' : 'text-gray-400'}`}>
                                Sala {sess.room}
                            </span>
                            
                            <span className={`text-3xl md:text-4xl font-black tracking-tighter ${isSelected ? 'text-blue-950' : 'text-white'}`}>
                                {sess.time}
                            </span>
