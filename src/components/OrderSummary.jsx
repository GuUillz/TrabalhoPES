...
const OrderSummary = ({ movie, session, seats, tickets, total, unitPrices }) => {
  if (!movie) return <div className="w-full h-full border-l border-white/10"></div>;

  // Usa os preços reais vindos do Backend ou valores padrão de segurança
  const priceInteira = unitPrices?.inteira || 30;
  const priceMeia = unitPrices?.meia || 15;
  const pricePromo = unitPrices?.promo || 10;
  ...
  return (
    <div className="w-full h-full flex flex-col text-white p-6">
      ...
      {/* Ingressos (CORRIGIDO: Usa preços dinâmicos) */}
      {tickets && (
        <div className="mb-4 text-xs space-y-2 border-t border-white/10 pt-4">
          {tickets.inteira > 0 && (
            <div className="flex justify-between">
              <span>{tickets.inteira}x INTEIRA</span>
              <span>R$ {(tickets.inteira * priceInteira).toFixed(2).replace('.', ',')}</span>
            </div>
          )}
          {tickets.meia > 0 && (
            <div className="flex justify-between">
              <span>{tickets.meia}x MEIA</span>
              <span>R$ {(tickets.meia * priceMeia).toFixed(2).replace('.', ',')}</span>
            </div>
          )}
          {tickets.promo > 0 && (
            <div className="flex justify-between">
              <span>{tickets.promo}x PROMOÇÃO</span>
              <span>R$ {(tickets.promo * pricePromo).toFixed(2).replace('.', ',')}</span>
            </div>
          )}
        </div>
      )}
